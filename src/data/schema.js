...
export async function deletePlayer(playerId) {
  ...
  for (const event of allEvents) {
    ...
    affectedEvents.push({ id: event.id, title: event.title, status: event.status })
  }
...
export async function cleanOrphanedSquadReferences() {
  const [allEvents, allPlayers] = await Promise.all([getEvents(), getPlayers()])
  ...
  for (const event of allEvents) {
    ...
    if (foundOrphan) {
      await updateEvent(event.id, { squads: cleanedSquads })
      affected.push({ id: event.id, title: event.title, status: event.status, removedIds })
    }
  }
...
export async function finalizeEvent(event, status, score = null) {
  ...
  for (const squad of event.squads || []) {
    for (const playerId of squad.memberIds || []) {
      if (squad.label === 'support') {
        updates.push(incrementWeeklyStat(weekKey, playerId, 'contributionPoints', 1))
      } else if (squad.label === 'scoring' && status === 'success') {
        updates.push(incrementWeeklyStat(weekKey, playerId, 'scoreAchieved', 1))
      }
    }
  }
  ...
  if (status === 'success' && score !== null && event.trialNumber) {
    const scoringSquad = (event.squads || []).find((s) => s.label === 'scoring')
...
export async function getWeeklyStats(weekKey) {
  const q = query(collection(db, 'weekly_stats'), where('weekKey', '==', weekKey))
  const snap = await getDocs(q)
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }))
}
...
export async function getTrialScoresForWeek(weekKey) {
  const q = query(collection(db, 'trial_scores'), where('weekKey', '==', weekKey))
  const snap = await getDocs(q)
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }))
}

export async function setTrialScore(weekKey, playerId, trialNumber, score) {
  const ref = doc(db, 'trial_scores', trialScoreId(weekKey, playerId, trialNumber))
  await setDoc(ref, {
    weekKey,
    playerId,
    trialNumber,
...
const DEFAULT_TRIAL_NAMES = { 1: 'Trial 1', 2: 'Trial 2', 3: 'Trial 3', 4: 'Trial 4', 5: 'Trial 5' }
...
export async function createRun({ weekKey, trialNumber, scoringMemberIds, supportMemberIds }) {
...
export async function finalizeRun(run, status, score = null) {
  await updateRun(run.id, { status, endedAt: serverTimestamp(), score: score ?? null })
  ...
  for (const playerId of run.supportMemberIds || []) {
    updates.push(incrementWeeklyStat(run.weekKey, playerId, 'contributionPoints', 1))
  }

  if (status === 'complete') {
    for (const playerId of run.scoringMemberIds || []) {
      updates.push(incrementWeeklyStat(run.weekKey, playerId, 'scoreAchieved', 1))
    }
    if (score !== null && run.trialNumber) {
      for (const playerId of run.scoringMemberIds || []) {
        updates.push(setTrialScore(run.weekKey, playerId, run.trialNumber, score))
      }
    }
  }
